...
- **`services/wallet-index.js`** — non-secret wallet listing `[{ address, name }]` in electron-store. Listing/renaming must NEVER enumerate keychain secrets (each secret read triggers one macOS ACL prompt per item); secrets are read one item at a time, on demand only. One-time migration from keychain on first run reads item ATTRIBUTES only via `security dump-keychain` (`services/keychain-list.js`, zero prompts) — never `keytar.findCredentials`, which decrypts every item. Migrated names default to the address; `healWalletName` adopts the real name whenever a secret is decrypted on demand, and `services/restore-names.js` (toolbar "Restore Names" button) bulk-restores all placeholders — one ACL dialog per wallet, progress persisted per wallet.
